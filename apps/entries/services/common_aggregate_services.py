...
class BaseProductionAggregateService:
    model_class = None  # Sẽ định nghĩa ở class con
    ...
    def aggregate_production_data(cls, queryset, filters):
        ...
        # Dùng cls.model_class thay vì BaggingInput
        lm_qs = cls.model_class.objects.filter(date__range=[s_dt - timedelta(days=30), e_dt - timedelta(days=30)])
        ly_qs = cls.model_class.objects.filter(
        ...
        for entry in queryset:
            ...
            for item in (entry.production_data or []):
                ...
                if iq == 0: lot_summary[l_no]["rw"] += oq
                else: lot_summary[l_no]["in"] += iq
                lot_summary[l_no]["out"] += oq
            lot_summary[l_no]["rj"] = lot_summary[l_no]["in"] - lot_summary[l_no]["out"]

        lot_chart_final = [{"name": k, "input": v["in"], "shipping": v["out"], "reject": v["rj"]}
                          for k, v in lot_summary.items()]

        return {
            "kpis": {
                "TOTAL IN": {"value": f"{curr['in']:,.0f}", "lastMonth": inst._diff(curr['in'], lm['in']), "lastYear": inst._diff(curr['in'], ly['in'])},
                "SHIPPING": {"value": f"{curr['ship']:,.0f}", "lastMonth": inst._diff(curr['ship'], lm['ship']), "lastYear": inst._diff(curr['ship'], ly['ship'])},
                "RATE": {"value": f"{curr['rate']:.1f}%", "lastMonth": inst._diff(curr['rate'], lm['rate']), "lastYear": inst._diff(curr['rate'], ly['rate'])},
                "WORKING TIME": {"value": f"{curr['time']:,.0f}h", "lastMonth": inst._diff(curr['time'], lm['time']), "lastYear": inst._diff(curr['time'], ly['time'])}
            },
            "charts": {
